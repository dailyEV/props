...
def writeHomerLogs():
	...
	for player, playerFeed in playerFeeds.items():
		btwn = 0
		hrs, closest = [], []
		close7, close5, close3 = 0,0,0
		dts = sorted(set([x[:10] for x,_,_ in playerFeed]))
		for dt_pa, result, hr_park in sorted(playerFeed):
			dt = datetime.strptime(dt_pa[:10], "%Y-%m-%d")
			if result == "Home Run":
				hrs.append((dt_pa, btwn))
				btwn = 0
			elif "2025" in dt_pa and int(hr_park.split("/")[0] or 0) > 0:
				closest.append(dt_pa)
				try:
					dtL3 = datetime.strptime(dts[-3], "%Y-%m-%d")
					dtL5 = datetime.strptime(dts[-5], "%Y-%m-%d")
					dtL7 = datetime.strptime(dts[-7], "%Y-%m-%d")
					if dt >= dtL3:
						close3 += 1
					if dt >= dtL5:
						close5 += 1
		...
		homerLogs[player] = {
			...
			"close": len(closest), "close3": close3, "close5": close5, "close7": close7
		}
		closeLogs[player] = {}

	with open("static/baseballreference/homer_logs.json", "w") as fh:
		json.dump(homerLogs, fh)
	with open("static/baseballreference/close_logs.json", "w") as fh:
		json.dump(closeLogs, fh)
...
def writePitcherPreview(date):
	...
	arr = []
	for team in lineups:
		pitcher = lineups[team]["pitcher"]
		adv = advanced.get(pitcher, {})
		adv["player"] = pitcher
		adv["team"] = team
